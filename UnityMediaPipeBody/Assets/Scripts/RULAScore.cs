...
public class RULAScore : MonoBehaviour
{
    ...
    public TMP_Text HeadRotationAngleText;
    //public float[][]
    ...
    void Update()
    {
        ...
        //  ======================================================================
        TrunkScore = 0;
        TrunkAngle = Mathf.Abs(LowerSpine.eulerAngles.x) + 2.5f;
        if (TrunkAngle > 360.0f)
        {
            TrunkAngle -= 360.0f;
        }
        if (TrunkAngle == 0.0f)
        ...
        int score = GetRULAScore(
            ...
            TableB[
                GetRowIndexTableB(NeckScore), GetColumnIndexTableB(TrunkScore, 1)
            ]
            );
    ...
    public int GetUpperArmScore(Transform arm)
    {
        ...
        if (calculator != null)
        {
            ...
            Debug.Log(arm.name + " - Arm Elevation: " + ArmElevation);
    ...
    {
        NeckAngle = neck.eulerAngles.x + 0.0f;
        if (NeckAngle >= 360.0f)
        {
            NeckAngle -= 360.0f;
        }
        //HeadRotationAngleText.text = "Angle: " + NeckAngle;
        NeckBend = neck.eulerAngles.z;
        Debug.Log("Neck Angle: " + NeckAngle);
