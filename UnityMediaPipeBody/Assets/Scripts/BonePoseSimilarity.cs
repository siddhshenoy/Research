...
public class BonePoseSimilarity : MonoBehaviour
{
    ...
    public float capStartTime = 0.0f;
    public float capThresh = 10.0f;
...
/*        if (OVRInput.GetDown(OVRInput.Button.Four))
        ...
        if (Input.GetKeyDown(KeyCode.Space))
        {
            if (!hasPressedSpace)
            {
                ...
                capStartTime = Time.time;
            }

        }
        if(hasPressedSpace)
        {
            if ((Time.time - capStartTime) > capThresh)
            {
            ...
            {
                TimerText.text = "Wait " + (capThresh - (Time.time - capStartTime)) + "seconds";
            }
        }
    }
